...
std::tuple<int, double> test_em(uint64_t nTests, bool reverse = false){
  ...
  std::cout<<std::endl;
  std::cout<<" (The above test results should match) "<<std::endl<<std::endl;
  
  double dif = 0;
  
  if(resultA < resultB){
    dif =(double)(resultB-resultA) / (double)resultB * 100;
    std::cout<<"  WINNER: TUPLE ("<<print(dif)<<"%"<<")"<<std::endl<<std::endl;
    winner = 1;
  }
    ...
    std::cout<<"  WINNER: STRUCT ("<<print(dif)<<"%"<<")"<<std::endl<<std::endl;
    winner = 2;
  }
